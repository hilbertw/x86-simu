...
void test_and8()
{
	...
	for (j=0;j<255;j++)
	{
			...
			||(cflag!=cflag1))
		{
			...
			printf("and8: %x,%x %x,%x (%x+%x)--",k,k1,cflag,cflag1,i,j);
			chk(O,err)
			chk(C,err)
			chk(Z,err)
			chk(S,err)
...
void test_and16()
{
	...
	for (j=0;j<0xffff;j++)
	{
			...
			||(cflag!=cflag1))
		{
			...
			printf("and16: %x,%x %x,%x (%x+%x)--",k,k1,cflag,cflag1,i,j);
			chk(O,err)
			chk(C,err)
			chk(Z,err)
			chk(S,err)
	...
	for (short j1=0;j1<0x100;j1++)
	{
		...
		for (short k3=0;k3<0x1000;k3++)
		{
				...
				||(cflag!=cflag1))
			{
				...
				printf("and32: %x,%x %x,%x (%x+%x)--",k,k1,cflag,cflag1,i,j);
				chk(O,err)
				chk(C,err)
				chk(Z,err)
				chk(S,err)
