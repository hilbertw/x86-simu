...
void test_neg8()
{
	...
	for (i=0;i<255;i++)
	{
			...
			||(cflag!=cflag1))
		{
			...
			printf("neg8: %x,%x %x,%x (%x+%x)--",k,k1,cflag,cflag1,i,j);
			chk(O,err)
			chk(C,err)
			chk(Z,err)
			chk(S,err)
...
void test_neg16()
{
	...
	for (i=0;i<0xffff;i++)
	{
			...
			||(cflag!=cflag1))
		{
			...
			printf("neg16: %x,%x %x,%x (%x+%x)--",k,k1,cflag,cflag1,i,j);
			chk(O,err)
			chk(C,err)
			chk(Z,err)
			chk(S,err)
...
void test_neg32()
{
	...
	for (short k3=0;k3<0x1000;k3++)
	{
			...
			||(cflag!=cflag1))
		{
			...
			printf("neg32: %x,%x %x,%x (%x+%x)--",k,k1,cflag,cflag1,i,j);
			chk(O,err)
			chk(C,err)
			chk(Z,err)
			chk(S,err)
